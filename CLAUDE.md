...
`demo/`, CI, `prek.toml` or this file is not one, and the version holds. The
level follows the change: a new command or flag is a minor while `0.x`, a fix a
patch.

The steps are in README.md under *Releasing*, and the shape is the one every
other Rust CLI uses: `cargo release version` and `cargo release replace` on a
`release/vX.Y.Z` branch, an ordinary pull request carrying the bump and the
dated changelog entry, then a tag pushed at the merged commit. Merging that pull
request is a deliberate act and yours, not something to arm auto-merge on.
Pushing the tag is what publishes, so it is yours too.

A version whose `## Unreleased` section is empty is a version with nothing to
announce, which is the same bar as cutting one at all.

Nothing is built or signed on the machine that pushes the tag. Do not build an
artifact or cut a release locally to work around a red workflow — fix the
workflow.
