...
auto-merge, then `make release-publish` hands the merged version to dist, which
builds every target before it creates the tag and the release. The level follows
the change: a new command or flag is a minor while `0.x`, a fix a patch.
