...
other Rust CLI uses: `cargo release version` on a `release/vX.Y.Z` branch, an
ordinary pull request carrying the bump, then a tag pushed at the merged commit.
Merging that pull request is a deliberate act and yours, not something to arm
auto-merge on. Pushing the tag is what publishes, so it is yours too.
...
that command, silently. `dist init` also rewrites `dist-workspace.toml`, keeping
only comments sitting directly above a key it still recognises.
