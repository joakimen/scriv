...
# and `[shell.aliases]` in your scriv config name the action each one runs.
# scriv binds nothing on its own, so an empty section here is an empty table
# there — `scriv config init` writes a set out commented to start from.
